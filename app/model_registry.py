...
PLANNED_MODELS: list[dict[str, Any]] = [
    ...
    {
        "id": "QWEN3_8_27B_Q6_K_L", "family": "Qwen3.8-27B", "stage": ["summarize", "actionize", "function_call"], "source_repository": "unsloth/Qwen3.8-27B-GGUF", "revision": "4ca720788d1e01f1bff70c033e0d0028fd02e502", "precision": "Q6_K_L", "runtime": "llama.cpp", "local_or_hosted": "LOCAL", "license_id": "Apache-2.0", "license_source": "https://huggingface.co/unsloth/Qwen3.8-27B-GGUF", "commercial_status": "PERMISSIVE", "redistribution_status": "PERMISSIVE", "teacher_use_status": "UNKNOWN", "training_eligibility": "UNKNOWN", "monetary_marginal_cost": 0, "artifact_root": "qwen/Qwen3.8-27B-Q6_K_L", "artifacts": [_artifact("D:/Qwen3.8-27B-UD-Q6_K_L.gguf", bytes_=24193919904, sha256="121355b4c7422771da25adc74090e3c90138f77ce5c92d348687d47824ec80f4")], "verified_capabilities": ["chat_completion", "structured_json_emulation", "arabic_action_extraction"], "runtime_certified": True, "capability_certified": False, "unverified_claims": ["multilingual tools", "native structured output", "native tool calls", "summarization"], "evidence_notes": "Pinned Q6_K_L artifact at the D: root; Apache-2.0 card metadata verified. Runtime load and JSON-emulation action extraction were exercised locally; native JSON grammar/tool calls remain unverified."},
    {
...
def scan_models(roots: list[Path] | None = None) -> list[dict[str, Any]]:
    ...
    for plan in PLANNED_MODELS:
        ...
        for spec in plan.get("artifacts", []):
            declared_path = model_dir / spec["path"]
            passive = passive_artifacts.get(str(spec.get("sha256", "")).lower()) or passive_artifacts.get(Path(spec["path"]).name.lower())
            passive_path = Path(str(passive["path"])) if passive else None
            # A stale download manifest must not mask a complete artifact that
            # arrived at the registry's explicitly declared path (including a
            # user-configured absolute path such as D:\\Qwen...).
            path = declared_path if declared_path.is_file() else passive_path or declared_path
